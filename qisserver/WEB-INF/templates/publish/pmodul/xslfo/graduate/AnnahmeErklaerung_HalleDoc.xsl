				...
				<fo:flow flow-name="Body" font-family="{$font}" font-size="10pt">
					...
					<fo:block-container absolute-position="fixed" top="119mm" left="30mm" width="170mm">
						<fo:block color="#000000" text-align="start" font-family="{$font}" font-weight="bold">
							Annahme als <xsl:value-of select="$dokorand" /> an der <xsl:value-of select="$fakName" />
						</fo:block>
					</fo:block-container>
					...
					<fo:block-container absolute-position="fixed" top="143mm" left="30mm" width="165mm">
						...
						<fo:block color="#000000" font-family="{$font}" text-align="justify" hyphenate="true" language="de" padding-top="1em">
							Die Durchsicht und Prüfung Ihrer eingereichten Unterlagen hat ergeben, dass die Zulassungsvoraussetzungen zur Promotion gemäß § 3 der Promotionsordnung erfüllt sind. Sie sind  hiermit als <xsl:value-of select="$dokorand" /> an der  <xsl:value-of select="$fakName" /> der Martin-Luther-Universität angenommen. 
						</fo:block>
						<fo:block color="#000000" font-family="{$font}" text-align="justify" hyphenate="true" language="de" padding-top="1em">
							Die Betreuung wird 
