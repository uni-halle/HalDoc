				...
				<fo:flow flow-name="Body" font-family="{$font}" font-size="10pt">
					<fo:block-container absolute-position="fixed" top="119mm" left="30mm" width="170mm">
						<fo:block color="#000000" text-align="start" font-family="{$font}" font-weight="bold">
							Ablehnung Ihres Antrages auf Annahme als <xsl:value-of select="$dokorand" />
						</fo:block>
					</fo:block-container>
					...
					<fo:block-container absolute-position="fixed" top="143mm" left="30mm" width="165mm">
						<fo:block color="#000000" font-family="{$font}" text-align="justify" hyphenate="true" language="de">
							Ihr Antrag auf Annahme als <xsl:value-of select="$dokorand" /> wurde in der Sitzung des Promotionsausschusses / des Wissenschaftlichen Rates am <xsl:value-of select="Person/MyPromo/Process/assumptionCommitee" /> behandelt.
						</fo:block>
						<fo:block color="#000000" font-family="{$font}" text-align="justify" hyphenate="true" language="de" padding-top="1em">
							Die Durchsicht und Prüfung Ihrer eingereichten Unterlagen hat ergeben, dass die Zulassungsvoraussetzungen zur Promotion gemäß § 3 der Promotionsordnung <xsl:if test="Person/MyPromo/Infos/Ablehnung">aufgrund <xsl:value-of select="Person/MyPromo/Infos/Ablehnung/txt" /></xsl:if> nicht erfüllt sind. Sie können deshalb nicht als <xsl:value-of select="$dokorand" /> an der Einrichtung <xsl:value-of select="Person/MyInfos/Einrichtung/ltxt" /> der Martin-Luther-Universität angenommen werden. 
						</fo:block>
						<fo:block color="#000000" font-family="{$font}" text-align="justify" hyphenate="true" language="de" padding-top="1em">
							Sie haben das Recht, gegen diese Entscheidungen des Promotionsausschusses Widerspruch einzulegen. Der Widerspruch ist innerhalb eines Monats nach Zugang der gefällten Entscheidung schriftlich oder zur Niederschrift bei der Dekanin bzw. beim Dekan der der Einrichtung <xsl:value-of select="Person/MyInfos/Einrichtung/ltxt" /> einzureichen.
						</fo:block>
