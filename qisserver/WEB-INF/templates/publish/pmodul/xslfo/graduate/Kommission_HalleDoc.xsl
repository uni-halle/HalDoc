				...
				<fo:static-content flow-name="KopfSeite1">
					...
					<fo:block-container position="absolute" left="24.5mm" top="60mm" width="85mm" height="38mm" border-top-style="solid" border-top-width="0.0pt" border-bottom-style="solid"
						...
						<fo:block text-align="start" font-size="10pt" line-height="14pt">
							<xsl:if test="Person/Form/Geschl='M'">
								<xsl:text>Herr </xsl:text>
							</xsl:if>
							<xsl:if test="Person/Form/Geschl='W'">
								<xsl:text>Frau </xsl:text>
							</xsl:if>
							<fo:block />
							<xsl:value-of select="Person/Form/Titel" />
							<xsl:text> </xsl:text>
							<xsl:value-of select="Person/Form/Vorname" />
							<xsl:text> </xsl:text>
							<xsl:value-of select="Person/Form/Nachname" />
							<fo:block />
							<xsl:value-of select="Person/Form/Strasse" />
							<fo:block />
							<xsl:value-of select="Person/Form/PLZ" />
							<xsl:text> </xsl:text>
							<xsl:value-of select="Person/Form/Ort" />
						</fo:block>
					</fo:block-container>
					<!-- NEU KOPF NEU KOPF -->
				</fo:static-content>
				...
				<fo:flow flow-name="Body" font-family="{$font}" font-size="10pt">
					<fo:block-container absolute-position="fixed" top="119mm" left="30mm" width="170mm">
						<fo:block color="#000000" text-align="start" font-family="{$font}" font-weight="bold">
							Mitglied in Promotionskommission
						</fo:block>
					</fo:block-container>
					...
					<fo:block-container absolute-position="fixed" top="143mm" left="30mm" width="165mm">
						<fo:block color="#000000" font-family="{$font}" text-align="justify" hyphenate="true" language="de">
							<xsl:if test="Person/Geschl='M'">
								<xsl:text>Herr </xsl:text><xsl:value-of select="Person/PerNac" /><xsl:text> hat seine </xsl:text>
							</xsl:if>
							<xsl:if test="Person/Geschl='W'">
								<xsl:text>Frau </xsl:text><xsl:value-of select="Person/PerNac" /><xsl:text> hat ihre </xsl:text>
							</xsl:if>
							Dissertation zum Thema "<xsl:value-of select="Person/MyPromo/Titel" />" der Einrichtung <xsl:value-of select="Person/MyInfos/Einrichtung/ltxt" /> vorgelegt und die Zulassung zum Promotionsverfahren beantragt. Dem Antrag wurde entsprochen.
						</fo:block>
						<fo:block color="#000000" font-family="{$font}" text-align="justify" hyphenate="true" language="de" padding-top="1em">
							Für Ihre Bereitschaft zur Mitwirkung in der Promotionskommission möchte ich Ihnen danken. Den Termin für die öffentliche Verteidigung stimmen wir mit Ihnen nach Eingang der Gutachten ab. 
						</fo:block>
