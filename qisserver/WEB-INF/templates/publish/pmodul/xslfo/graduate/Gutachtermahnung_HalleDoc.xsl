					...
					<fo:block-container absolute-position="fixed" top="143mm" left="30mm" width="165mm">
						<fo:block color="#000000" font-family="{$font}" text-align="justify" hyphenate="true" language="de">
							...
							<xsl:if test="$printLang='de'">
								...
								<!-- 
								da Sie Gutachter im Promotionsverfahren von <xsl:value-of select="$doktorand" /> sind, erlaube ich mir bei Ihnen anzufragen, wann mit der Übersendung Ihres Gutachtens zu rechnen ist, damit wir das Promotionsverfahren entsprechend der Promotionsordnung weiterführen können.
								 -->
								<xsl:variable name="text"><xsl:value-of select="Person/Form/Text" /></xsl:variable>
								<xsl:value-of select="strClass:replaceAll($text,'\$doktorand', $doktorand)" />
							</xsl:if>
						</fo:block>
